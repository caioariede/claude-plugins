...
5. **`--plan` → planned unit.** Append `- [ ] <slug>  base=<base>  [needs=<a>[,<b>]]  — <what>` to `## Planned units`. This is a **dependency reservation** — it records stack/need fields for a future `ws-start`, not a roadmap entry that `ws-next` will auto-start. `slug = slug(what)`; `base` = `--base` (a unit-id stacks, else a branch), default the active `forge` flavor's `default-branch`. Validate each `--needs` target for self-need / cycle as `ws-start` does — skip and warn on a bad one. **Refuse** if a ledger unit already matches `<slug>` (already started; the line would derive-done at once — SPEC dedup-vs-ledger).
