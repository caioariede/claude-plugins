...
3. Create the worktree via the active `worktree-management` flavor's `create` (SPEC §Flavors), for branch `<slug>` off `<base>`; then run `open-window` if the flavor defines it. Disambiguate the branch with `-N` if `<slug>` already exists in the target repo (per SPEC). Do not steal the current session's focus.
