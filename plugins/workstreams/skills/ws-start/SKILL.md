...
## Steps
1. Resolve `ws-id` → `~/.claude/workstreams/<ws-id>/`. Compute `unit-id = slug(what)`. If `units/<unit-id>/` already exists → **confirm**: resume the existing unit (`ws-resume`) or start fresh. A fresh start takes the next `-N` suffix and records `restart-of=<original-unit-id>` on its ledger line (per SPEC).
...
4. **Append** the ledger line to `units.md` (SPEC format; include `restart-of=` / `stacked-on=` when applicable).
