...
6. The unit is provisioned and its intent is in `charter.md` — do **not** print a bootstrap prompt. Tell the user to run **`/ws-resume`** — from the current session, or in the `feat-<slug>` window for a parallel session. That is the single verb from here on — it reads `charter.md` + the design and **plans** an unplanned unit (writing `T1..` into `progress.md`), **continues** a half-done one, and **ships** a finished one.
