...
Headless runs inline this rule when hooks skip.
...
- `spec-driven-development` — `plan` (charter+design → `T1..`) · `execute` (first unchecked task) · `ship` (open the PR) · `spec-glob` (optional — glob of this flavor's design-spec paths; powers Spec-watch below) · `plan-glob` (optional — glob of implementation-plan paths; powers Plan-watch below).
