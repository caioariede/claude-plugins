...
## Steps
1. Resolve the unit via SPEC §IDs & conventions (bare-slug resolver) → `branch`, worktree, `repo`, current PR. Runs from any session: operate on the worktree via `git -C <worktree>` (SPEC §Command scope) — do not move the current session.
