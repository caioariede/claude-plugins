...
4. Append a `dropped <reason>` log line per SPEC File formats. **Keep** `progress.md` and the roster line — they are the reconciliation record. The unit's **deferred** follow-ups already live in `backlog.md` and survive the drop; its in-flight `progress.md` follow-ups go dormant with it.
