...
3. If the unit has a wmx worktree, `wmx worktree remove <branch>` (removes worktree + its window); otherwise `git worktree remove <path>`. Then delete the **local** branch. Do **not** delete the remote branch or close the PR unless the user asks.
