...
execute. On `ship-detect-candidate`, print the **ship-detect gate**
(§Pause gates) before continuing. Chain to `ws-next` when `done`
(§Next).
...
**drift gate** — only when step 5 printed `split pr=#N commits=M`. Context block has no ordinals:
...
**ship-detect gate** — only when step 2 printed `ship-detect-candidate
