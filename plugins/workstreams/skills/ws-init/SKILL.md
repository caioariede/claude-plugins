...
2. Create `<store>/<ws-id>/workstream.md` (SPEC §Store layout) in SPEC §File formats (`workstream.md`). Set `goal` to a one-line restatement of the name; ask the user for the goal only if `$ARGUMENTS` is empty or a single word. Set `design:` to an umbrella spec path only if one exists.
...
4. Report the `id`. Offer `ws-focus` now (SPEC §Next-step chaining — store-only). Do not collect the outcome here; `ws-focus` owns the prompt and `add`. On decline, stop — do not offer `ws-next`.
